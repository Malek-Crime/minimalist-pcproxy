...
static pcp_proxy_request get_request(pcp_common_header h)
{
  ...
  for (i = 0; i < NUM_REQUESTS; i++) {
    ...
    if (req->t && memcmp(req->nonce, nonce, PCP_NONCE_LENGTH) == 0)
      return req;
  }
  return NULL;
}
                                          ...
                                          pcp_common_header h)
{
  int i;
  time_t now = get_time();
  time_t old_time = now - PCP_PROXY_ASSUMED_REQUEST_ROUNDTRIP_SECONDS;
  time_t t;
                          ...
                          struct sockaddr_in6 *address)
{
  ...
  if (!s) {
    DEBUG("Failed to allocate server");
    return;
  }
  s->prefix = *prefix;
  s->plen = plen;
...
static int parse_sockaddr_in6(const char *c,
                              struct sockaddr_in6 *sin6, uint16_t dport)
{
  /* Parse the address or [address]:port in c to sin6.
   * Do not touch anything except sin6_addr and sin6_port (if available). */
  if (!c || !sin6)
    return 0;
  ...
  char *p1 = STRCHRISH(buf, '[');
  char *p2 = STRCHRISH(p1, ']');
  char *p3 = p2 ? STRCHRISH(p2, ':') : NULL;
  ...
  if (p1 && p2 && p3) {
    ...
    *p2 = 0;
    ...
    /* Check if it's a simple IPv4:port format */
    char *colon = strrchr(buf, ':');
    if (colon && strchr(buf, ':') == colon) {
      /* Only one colon, could be IPv4:port */
      *colon = 0;
      host = buf;
      port = colon + 1;
    } else {
      /* IPv6 or IPv4 without port */
      host = buf;
      port = NULL;
    }
  }
  ...
  ret = in6_pton(host, &sin6->sin6_addr);
  free(buf);
  return ret;
}
                                 ...
                                 char *err, size_t err_len)
{
  ...
  char *c = d ? STRCHRISH(d, '=') : NULL;
  
  if (!c || !d) {
    snprintf(err, err_len, "Invalid server format (expected X/Y=Z)");
    free(bases);
    return false;
  }
  ...
  *c = 0;
  ...
  if (in6_pton(prefix, &p) < 1) {
    snprintf(err, err_len, "Unable to parse prefix: %s", strerror(errno));
    free(bases);
    return false;
  }
  
  if (parse_sockaddr_in6(c, &sin6, PCP_SERVER_PORT) < 1) {
    snprintf(err, err_len, "Unable to parse server address: %s", strerror(errno));
    free(bases);
    return false;
  }
  ...
  if (plen < 0 || plen > 128) {
    snprintf(err, err_len, "Invalid prefix length: %ld", plen);
    free(bases);
    return false;
  }
  
  pcp_proxy_add_server(&p, (int)plen, &sin6);
  free(bases);
  return true;
}
...
static pcp_proxy_server determine_server_for_source(struct in6_addr *src)
{
  ...
  list_for_each_entry(s, &servers, lh) {
    ...
    if (whole > 0 && memcmp(s->prefix.s6_addr, src->s6_addr, whole))
      continue;
...
for (o = (pcp_option)(data) ;                           \
     ((void *)o + sizeof(*o)) <= ((void *)(data) + (data_len)) \
       && ((void *)o + sizeof(*o) + ntohs(o->len))     \
           <= ((void *)(data) + (data_len)) ;           \
     o = (pcp_option)((uint8_t *)o + sizeof(*o) + ntohs(o->len)))
...
pcp_thirdparty_option find_third_party_option(void *data, int data_len)
{
  ...
  int remaining = data_len - (ptr - data);
  if (remaining <= 0)
    return NULL;

  for_each_pcp_option(o, ptr, remaining) {
    if (o->option_code == PCP_OPTION_THIRD_PARTY &&
        ntohs(o->len) == 16)
...
{
  if (!src || !dst || !data || data_len < (int)sizeof(pcp_common_header_s))
    return;
                                  ...
                                  void *data, int data_len)
{
  ...
  case PCP_OPCODE_ANNOUNCE:
    ...
    {
      pcp_option o;
      int remaining = data_len - sizeof(*h);
      for_each_pcp_option(o, (uint8_t *)data + sizeof(*h), remaining) {
                                  ...
                                  void *data, int data_len)
{
  ...
  if (!req->third_party) {
     ...
     * rid of it. */
    int tpo_size = sizeof(*tpo) + ntohs(tpo->po.len);
    int remaining = data_len - ((uint8_t *)tpo - (uint8_t *)data) - tpo_size;
    
    if (remaining > 0) {
      memmove(tpo, (uint8_t *)tpo + tpo_size, remaining);
    }
    data_len -= tpo_size;
  }
